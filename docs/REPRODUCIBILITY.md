...
The command fails closed if required data/cache/run/map/report directories or their manifest hashes are absent. It never downloads datasets or weights. It writes `reproduction_manifest.json` and per-stage command logs atomically under the output root.
