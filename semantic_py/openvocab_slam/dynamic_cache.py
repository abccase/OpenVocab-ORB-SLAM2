...
class DynamicCacheManifest:
    ...
    def from_primitive(cls, value: dict[str, Any]) -> "DynamicCacheManifest":
        required = {
            ...
            "semantic_index_sha256", "source_tree_sha256", "association_sha256",
        ...
        if set(value) != required or not isinstance(value["dynamic_config"], dict):
...
def build_dynamic_job(
    ...
    *,
    ...
    _validate_bootstrap_manifest(run_manifest, sequence_id, trajectory)
    _read_trajectory(trajectory)
    config_value = cfg.to_primitive()
    semantic_entries = _read_jsonl(semantic_index_path)
    expected_instance_count = sum(
        len(read_cache_frame(semantic_root / str(entry["path"])).instances)
...
def generate_dynamic_cache(job: DynamicCacheJob) -> DynamicCacheResult:
    ...
    for frame_id, association_row in enumerate(associations):
        timestamp, rgb_relative, _depth_timestamp, depth_relative = association_row
        ...
        if depth is None or depth.ndim != 2 or depth.shape != (packet.image_height, packet.image_width):
    ...
    return DynamicCacheResult(cache_root, tuple(frame_index), tuple(track_rows))
...
def validate_dynamic_cache(
    ...
    *,
    ...
    try:
        ...
        if len(rows) != expected.expected_instance_count:
            raise ValueError(
                f"track row coverage mismatch: {len(rows)} != {expected.expected_instance_count}"
            )
        for row in rows:
            ...
            if not 0.0 <= probability <= 1.0 or bool(row["strong_dynamic"]) and probability < 0.70:
...
def _validate_bound_inputs(job: DynamicCacheJob) -> None:
    ...
    if run_manifest.get("state") != "COMPLETED" or run_manifest.get("valid") is not True:
        raise ValueError("bootstrap run manifest is not valid and complete")
    _validate_bootstrap_manifest(run_manifest, job.sequence_id, job.trajectory_path)


def _validate_bootstrap_manifest(
    manifest: dict[str, Any], sequence_id: str, trajectory_path: Path
) -> None:
    ...
    if not isinstance(trajectory, dict) or trajectory.get("sha256") != _sha256_file(trajectory_path):
