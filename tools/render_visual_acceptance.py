...
def online_plot_svg(report: dict[str, Any], telemetry_rows: list[dict[str, Any]]) -> str:
    ...
    def bars(values: list[float], maximum: float, x: int, title: str, labels: list[str]) -> str:
        return f'<text x="{x}" y="75" font-family="sans-serif">{title}</text>' + "".join(f'<rect x="{x+i*55}" y="{190-100*v/maximum:.1f}" width="35" height="{100*v/maximum:.1f}" fill="#1f77b4"/><text x="{x+i*55}" y="208" font-size="10">{labels[i]}</text>' for i,v in enumerate(values))
    timeline = "".join(f'<rect x="{60 + 800*i/max(1,len(telemetry_rows)):.1f}" y="390" width="{800/max(1,len(telemetry_rows)):.1f}" height="28" fill="{"#d62728" if row.get("semantic_state") == "DEGRADED_TO_BASELINE" else "#2ca02c"}"/>' for i,row in enumerate(telemetry_rows))
    body = bars(hz, max(5.0, max(hz)), 60, "rate (Hz): cap / actual / accepted", ["cap","actual","accepted"]) + bars(ms, max(1.0,max(ms)), 310, "latency (ms): mean / p95", ["mean","p95"]) + bars(frac, 1.0, 560, "fractions (0-1): request-drop / unusable / degraded", ["drop","unusable","degraded"]) + '<text x="60" y="375">frame timeline: green ONLINE_VALID, red DEGRADED_TO_BASELINE</text>' + timeline
...
def render(asset_root: Path, output: Path) -> dict[str, Any]:
    ...
    online_root = asset_root / "runs/p06-online/fr3_walking_xyz/seed-23011/demo/attempt-006"
    online = json.loads((online_root / "online_demo_report.json").read_text(encoding="utf-8"))
    import csv
    with (online_root / "frame_telemetry.csv").open(encoding="utf-8", newline="") as stream:
        online_rows = list(csv.DictReader(stream))
    sources += [online_root / "online_demo_report.json", online_root / "frame_telemetry.csv", online_root / "run_manifest.json"]
...
<section><h2>6. P06 online timing / rate / latency / drops</h2><img src="{svg_uri(online_plot_svg(online, online_rows))}" alt="online timing"><p>Run ID {online['run_id']}: request cap 5 Hz, actual request rate {online['request_attempt_rate_hz']:.3f} Hz, accepted semantic rate {online['accepted_semantic_rate_hz']:.3f} Hz, mean inference {online['mean_service_inference_ms']:.1f} ms, p95 {online['p95_service_inference_ms']:.1f} ms, maximum IPC call {online['maximum_ipc_call_ms']:.3f} ms, request-drop fraction {online['request_drop_fraction']:.3f}, unusable-result fraction {online['result_unusable_fraction']:.3f}, degraded frame fraction {online['degraded_frame_fraction']:.3f}. This is a hardware-limited degraded-only demonstration.</p></section>
    ...
    delivery = {
        ...
        "study": {"valid_runs": int(summary.get("paired_statistics", {}).get("overall", {}).get("pair_count", 0)) * 2, "outcome": summary.get("outcome_classification"), "summary_sha256": sha256(asset_root / "reports/final/summary.json"), "run_registry_sha256": sha256(asset_root / "runs/ovorb2_tum_v1/run_registry.jsonl")},
        "test_build_identity": {"python_tests": "256 collected at fix review", "ctest": "8/8", "cmake_cache_sha256": sha256(ROOT / "build/CMakeCache.txt") if (ROOT / "build/CMakeCache.txt").is_file() else None},
